...
const DetectionsPage: React.FC = () => {
  ...
  const { data, isLoading, error } = useQuery<PaginatedResponse<DetectionResponse>, Error>({
    queryKey: ['dets', page, label, minConf],
    queryFn: () => listDetections({ page, page_size: 5, label: label || undefined, min_confidence: minConf === '' ? undefined : Number(minConf) }),
