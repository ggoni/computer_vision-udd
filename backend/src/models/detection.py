...
class Detection(BaseModel):
    ...
    label: Mapped[str] = mapped_column(
        String(100),
        nullable=False,
        doc="Detected object label (e.g., 'cat', 'car', 'person')",
    )

    confidence_score: Mapped[float] = mapped_column(
        Float, nullable=False, doc="Model confidence score (0.0 to 1.0)"
    )

    # Bounding box coordinates (absolute pixel values)
    bbox_xmin: Mapped[int] = mapped_column(
        Integer, nullable=False, doc="Left edge of bounding box (x-coordinate)"
    )

    bbox_ymin: Mapped[int] = mapped_column(
        Integer, nullable=False, doc="Top edge of bounding box (y-coordinate)"
    )

    bbox_xmax: Mapped[int] = mapped_column(
        Integer, nullable=False, doc="Right edge of bounding box (x-coordinate)"
    )

    bbox_ymax: Mapped[int] = mapped_column(
        Integer, nullable=False, doc="Bottom edge of bounding box (y-coordinate)"
    )
